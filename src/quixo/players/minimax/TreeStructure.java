...
//		maxNode=minmaxTree.start(mainNode, actualDepth);
	...
	//	System.out.println(mainNode);
		...
		System.out.println("gyoker "+mainNode.index+" ertek "+mainNode.value+" level "+mainNode.leaf);
		
		System.out.println(maxNode);
		System.out.println("maxNode: "+maxNode.value+" lepes "+maxNode.step+" level "+maxNode.leaf);
		
	//	if(maxNode.value==0){
			find(maxNode);
		/*	while(!maxNode.isEnd() || !maxNode.isLeaf() || !maxNode.children.isEmpty()){
				for(Node n: maxNode.children){
					if(n.value==maxNode.value){
						System.out.println("ertek "+n.value+"lepes "+n.step);
						//minmaxTree=new Minimax(maxNode);
						maxNode=n;
					}
				}
			}
	...
	public void find(Node node){
		for(Node n: node.children){
			if(n.value==node.value){
				System.out.println("ertek "+n.value+" lepes "+n.step+" level "+maxNode.leaf);
				if(!n.isEnd() || !n.isLeaf() || !n.children.isEmpty()){
					find(n);
					return;
				}	
			}
